...
import {
  Users, Search, Plus, Eye, Edit, ChevronRight, ChevronLeft,
  GraduationCap, UserCheck, FileText, HeartPulse, ArrowUpRight,
  Clock, CheckCircle2, AlertTriangle, XCircle, Download, Upload,
  Phone, Mail, MapPin, Calendar, BookOpen, Award, Shield,
  Building2, MessageSquare, ClipboardList, IndianRupee, Activity,
  Printer, Send, Flag, Hash, Stethoscope, Syringe, Eye as EyeIcon,
  Ruler, Weight, Droplets, AlertCircle, UserX, PenTool, BarChart3,
  PieChart as PieChartIcon, Armchair
} from 'lucide-react'
import {
...
export default function SISModule() {
  ...
  const [admissionFormData, setAdmissionFormData] = useState({
    ...
    previousSchool: '', previousClass: '', previousTC: '', address: '', city: '', state: 'West Bengal', pincode: '',
          ...
          {showAdmissionForm && (
            <motion.div variants={itemVariants} className="rounded-2xl border border-border bg-card p-5">
              ...
              <div className="space-y-4">
                ...
                {/* Previous School */}
                <div>
                  <h4 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2"><Building2 className="w-4 h-4 text-birla-gold" /> Previous School</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div><label className={labelClass}>Previous School Name</label><input type="text" value={admissionFormData.previousSchool} onChange={(e) => setAdmissionFormData({...admissionFormData, previousSchool: e.target.value})} className={inputClass} placeholder="Previous school name" /></div>
